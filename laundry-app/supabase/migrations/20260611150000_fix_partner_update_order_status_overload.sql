-- PostgREST cannot disambiguate two overloads when p_assigned_rider_id is omitted or null.
-- Keep a single function; rider assignment on accept is optional via p_assigned_rider_id.

drop function if exists public.partner_update_order_status(uuid, text, integer, text, text);
drop function if exists public.partner_update_order_status(uuid, text, integer, text, text, uuid);
